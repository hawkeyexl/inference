...
The CLI's JSON envelope had no string `result`.

<Aside type="caution" title="Malformed CLI output surfaces as a raw SyntaxError">
  If the CLI prints something that is not JSON at all — an update banner, a login prompt — the
  envelope parse throws before this check, and a bare `SyntaxError: Unexpected token …` reaches
  `run.error`. Unmistakable once you know to expect it.
</Aside>
